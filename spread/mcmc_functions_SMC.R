# Inherits from mcmc_functions.R. The update_ranef function is modified, as the
# plaholder prior from stage1 is now always a logistic(0, 1) density at the
# fire-wise logit scale.
...
# Returns a vector of updated parameters, as c(b, s2). [b includes intercept]
...
update_ranef <- function(Y, mu, Sigma, Ytry, 
  ...
  for(j in 1:J1) {
    ...
    # Using SMC in stage 1, the priors were flat in the constrained space,
    # which implies logistic (0, 1) in the logit scale
    phpri_new <- 
      dlogis(Ynew_pri1, 0, 1, log = T) |> sum() +
      logit_scaled_d_log(steps_new, steps_bounds[j, 1], steps_bounds[j, 2])
    
    phpri_old <- 
      dlogis(Yold_pri1, 0, 1, log = T) |> sum() +
