# Inherits from <hierarchical model fitting_FWIZ2.R>. 
# Now, use samples from SMC.
...
source(file.path("spread", "mcmc_functions_SMC.R"))

source(file.path("flammability indices", "flammability_indices_functions.R"))
...
data_summ <- readRDS(file.path(
  "data", "flammability indices", "ndvi_elevation_summary.rds"
))
...
size_data <- readRDS(file.path(
  "data", "focal fires data", "fire_size_data.rds"
))
...
#         file.path("files", "hierarchical_model_FWIZ_SMC", "fwi_mean_sd_spread.rds"))
###
...
# saveRDS(
#   stansteps,
#   file.path("files", "hierarchical_model_FWIZ_SMC", "steps_model_stan_samples.rds")
# )
stansteps <- readRDS(
  file.path("files", "hierarchical_model_FWIZ_SMC", "steps_model_stan_samples.rds")
)
...
# }
# saveRDS(par_start, file.path("files", "hierarchical_model_FWIZ_SMC", "par_start.rds"))
par_start <- readRDS(file.path("files", "hierarchical_model_FWIZ_SMC", "par_start.rds"))
...
saveRDS(run0, file.path("files", "hierarchical_model_FWIZ_SMC", "run0.rds"))
run0 <- readRDS(file.path("files", "hierarchical_model_FWIZ_SMC", "run0.rds"))
...
saveRDS(sd_jump_tune, file.path("files", "hierarchical_model_FWIZ_SMC", "sd_jump_tune.rds"))

# Run MCMC in parallel ----------------------------------------------------

run0 <- readRDS(file.path("files", "hierarchical_model_FWIZ_SMC", "run0.rds"))
sd_jump_tune <- readRDS(file.path("files", "hierarchical_model_FWIZ_SMC", "sd_jump_tune.rds"))
...
for(batch in 1:nb) {
  ...
  if(batch == 1) {
    ...
    draws_prev <- readRDS(file.path("files", "hierarchical_model_FWIZ_SMC", fname_prev))
    # get last iters
  ...
  saveRDS(draws, file.path("files", "hierarchical_model_FWIZ_SMC", fname))
}
Sys.time()
...
ff <- list.files(file.path("files", "hierarchical_model_FWIZ_SMC"), pattern = "draws_batch_")
nsave <- 150

# assign growing iter_id to samples from consecutive batches
dlist <- lapply(1:length(ff), function(i) {
  x <- readRDS(file.path("files", "hierarchical_model_FWIZ_SMC", ff[i]))
...
saveRDS(draws, file.path(
  "files", "hierarchical_model_FWIZ_SMC", "spread_model_samples.rds"
))
...
draws <- readRDS(file.path(
  "files", "hierarchical_model_FWIZ_SMC",
...
ncomb <- nrow(combs)
combs$num1 <- as.numeric(factor(combs$V1, levels = par_names))
combs$num2 <- as.numeric(factor(combs$V2, levels = par_names))
...
for(i in 1:ncomb) {
  ...
  vv <- c(v1, v2)
  vv_num <- c(v1_num, v2_num)
  for(p in 1:2) {
    if(vv[p] != "steps") {
      sims_marg2[, p, ] <- invlogit_scaled(sims_marg[, p, ], 
                                           Lpar[vv_num[p]], Upar[vv_num[p]])
      sims_cond2[, p, ] <- invlogit_scaled(sims_cond[, p, ], 
                                           Lpar[vv_num[p]], Upar[vv_num[p]])
    } else {
      for(j in 1:npost) {
        sims_marg2[, p, j] <- invlogit_scaled(sims_marg[, p, j], stepsL, 
                                             draws$stepsU[j])
        sims_cond2[, p, j] <- invlogit_scaled(sims_cond[, p, j], stepsL, 
                                              draws$stepsU[j])
      }
    }
  }
...
saveRDS(mu_samples, file.path("files", "hierarchical_model_FWIZ_SMC", "mu_samples_prediction.rds"))
mu_samples <- readRDS(file.path("files", "hierarchical_model_FWIZ_SMC", "mu_samples_prediction.rds"))
...
# Constrain (steps already constrained)
ranef_cons <- draws$ranef
for(i in 1:npost) {
  ranef_cons[1:(n_coef-1), , i] <- invlogit_scaled2(
...
steps_summ <- apply(steps_all_draws, 1, summarise) |> t() |> as.data.frame()
...
steps_log_summ <- apply(log(steps_all_draws), 1, summarise) |> t() |> as.data.frame()
...
saveRDS(curves_df, file.path("files", "hierarchical_model_FWIZ_SMC", "curves_df_prediction.rds"))
curves_df <- readRDS(file.path("files", "hierarchical_model_FWIZ_SMC", "curves_df_prediction.rds"))
...
saveRDS(curves_df, file.path("files", "hierarchical_model_FWIZ_SMC", "curves_df_prediction_raw_x.rds"))
curves_df <- readRDS(file.path("files", "hierarchical_model_FWIZ_SMC", "curves_df_prediction_raw_x.rds"))
...
ranef_fit <- draws$ranef[, , ids_sim]
for(j in 1:nsim) {
  ranef_fit[1:(n_coef-1), , j] <- invlogit_scaled2(
    t(draws$ranef[1:(n_coef-1), , ids_sim[j]]),
    Lpar, Upar
  ) |> t()
}
...
for(j in 1:nsim) {
  jj <- ids_sim[j]
  mumat <- X_ %*% t(draws$fixef[1:n_coef, c("a", "b"), jj])
  
  # Compute Vcov
  sds <- draws$fixef[1:n_coef, "s2", jj] |> sqrt()
  V <- diag(sds) %*% draws$rho[, , jj] %*% diag(sds)
  
  sims_logit <- mgcv::rmvn(J1, mumat, V)
  
  # constrain
  Upar_[n_coef] <- draws$stepsU[jj]
  ranef_sim[, , j] <- invlogit_scaled2(sims_logit, Lpar, Upar_) |> t()
}

# merge to simplify paralelization
ranef_both <- abind::abind(ranef_fit, ranef_sim, along = 3)
ranef_both <- aperm(ranef_both, c(3, 1, 2))
dimnames(ranef_both) <- list(
 "sim" = 1:(nsim*2),
 "parameter" = par_names,
 "fire_num" = 1:J1
)
...
saveRDS(metrics_table, file.path("files", "hierarchical_model_FWIZ_SMC", "metrics_table.rds"))
saveRDS(size_obs, file.path("files", "hierarchical_model_FWIZ_SMC", "size_obs.rds"))
saveRDS(veg_available, file.path("files", "hierarchical_model_FWIZ_SMC", "veg_available.rds"))

metrics_table <- readRDS(file.path("files", "hierarchical_model_FWIZ_SMC", "metrics_table.rds"))
size_obs <- readRDS(file.path("files", "hierarchical_model_FWIZ_SMC", "size_obs.rds"))
veg_available <- readRDS(file.path("files", "hierarchical_model_FWIZ_SMC", "veg_available.rds"))
...
veg_levels_all <- c("All vegetation types", as.character(veg_levels))
unif_q <- ppoints(J1)
...
res_table$veg_class <- factor(as.character(res_table$veg_class),
                              levels = c(as.character(veg_levels), 
                                         veg_levels_all[1]))
...
# Size quotient distribution ___________________________

qfit <- t(metrics_table[ids_fit, "size", ]) / size_obs[, 1]
qsim <- t(metrics_table[ids_sim, "size", ]) / size_obs[, 1]

qsumm <- rbind(
  cbind(apply(qfit, 1, summarise) |> t() |> as.data.frame(), 
        size_obs = size_obs[, 1]),
  cbind(apply(qsim, 1, summarise) |> t() |> as.data.frame(),
        size_obs = size_obs[, 1])
)
qsumm$ranef <- factor(
  rep(c("Fitted parameters", "Simulated parameters"), each = J1),
  levels = c("Fitted parameters", "Simulated parameters")
)
qsumm$fire_id <- factor(
  rep(fire_ids, 2),
  levels = fire_ids[order(size_obs[, 1])]
)

ggplot(qsumm, aes(fire_id, mean, ymin = hdi_lower_95, ymax = hdi_upper_95)) + 
...
# The same in log
qsumm_log <- rbind(
  cbind(apply(log(qfit), 1, summarise) |> t() |> as.data.frame(), 
        size_obs = size_obs[, 1]),
  cbind(apply(log(qsim), 1, summarise) |> t() |> as.data.frame(),
        size_obs = size_obs[, 1])
)
qsumm_log$ranef <- factor(
  rep(c("Fitted parameters", "Simulated parameters"), each = J1),
  levels = c("Fitted parameters", "Simulated parameters")
)
qsumm_log$fire_id <- factor(
  rep(fire_ids, 2),
  levels = fire_ids[order(size_obs[, 1])]
)

ggplot(qsumm_log, aes(fire_id, mean, ymin = hdi_lower_95, ymax = hdi_upper_95)) + 
  geom_hline(yintercept = 0, color = viridis(1, begin = 0.7)) + 
  geom_linerange(alpha = 0.5) +
  geom_point(size = 2, alpha = 0.7) +
  facet_wrap(vars(ranef), nrow = 2, scales = "free_y", 
             axes = "all", axis.labels = "margins",
...
(
  p1 <-
    ...
    annotate("text", x = -0.57, y = 0.117, label = "A", size = 12/.pt, 
