...
# ~ fwi fortnight
ggplot(posteriors_summ) +#[!posteriors_summ$too_wide, ]) +
  geom_smooth(aes(fwi_fort_expquad, y = mean), method = "lm") +
  geom_linerange(aes(fwi_fort_expquad, ymin = hdi_lower_95, ymax = hdi_upper_95), alpha = 0.7) +
  # geom_linerange(aes(fwi_fort_expquad, ymin = hdi_lower_80, ymax = hdi_upper_80), alpha = 0.7) +
  geom_point(aes(fwi_fort_expquad, y = mean), alpha = 0.7) +
  facet_wrap(vars(par_name), ncol = 3, scales = "free_y") +
...
# GAM
ggplot(posteriors_summ[!posteriors_summ$too_wide, ]) +
  geom_smooth(aes(fwi_fort_expquad, y = mean), method = "gam") +
  geom_linerange(aes(fwi_fort_expquad, ymin = hdi_lower_95, ymax = hdi_upper_95), alpha = 0.7) +
  geom_linerange(aes(fwi_fort_expquad, ymin = hdi_lower_80, ymax = hdi_upper_80), alpha = 0.7) +
  geom_point(aes(fwi_fort_expquad, y = mean), alpha = 0.7) +
  facet_wrap(vars(par_name), ncol = 3, scales = "free_y") +
...
# fwi model
mm <- lm(mean ~ par_name * fwi_fort_expquad, data = cordata)
...
plot_cor_marg <- ggarrange2(plots = plots_list_marg,
                            layout = lay)
ggsave("spread/figures/pairs_plot_marginal_FWIZ.png", plot = plot_cor_marg,
       width = 20, height = 20, units = "cm")


plot_cor_cond <- ggarrange2(plots = plots_list_cond,
                            layout = lay)
ggsave("spread/figures/pairs_plot_conditional_FWIZ.png", plot = plot_cor_cond,
       width = 20, height = 20, units = "cm")