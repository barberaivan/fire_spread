...
# In the first model fitted, the steps followed a lognormal regression, with a 
# linear effect of FWI at the log scale. However, that caused the model to
# simulate absurdly large steps for FWI values outside the data range. Hence,
# a second model is defined, assuming a scaled-logit-normal distribution of the
# steps, with a logit-linear effect of the FWI (linear at logit scale).
# The lower limit for the steps is fixed at 2, so the spread model can always 
# spread at least one cell from the ignition cell (ignition is considered as
# step 1) (L = 2). The upper limit is set to U = 2000, which implies that a 
# fire can spread at most, 2000 * 30 = 60000 m (60 km) from the ignition point.
# That is approximately the width of the Andean-Patagonian forest. 
# However, the upper limit, U, is estimated, bounded between Umin and 
# Umax = 2000.
# In the simplest case, Umin is fixed at the maximum of step samples obtained in
# stage 1. However, we could also define it as a transformed parameter, being
# the maximum observed in each MCMC iteration.
# U will be sampled at the logit scale, with a logistic(0, 1) prior.

# All functions modified for the logitnormal steps model are named with 2 in 
# the end.
...
## (transforms below are likely not used)
...
# Translate steps parameter from logit scale at shared bounds [A, B] to logit scale
# at fire-wise bounds [C, D].
logit_translate <- function(x, A, B, C, D) {
  x1 <- plogis(x) * (B-A) + A # common constrained scale
  x2 <- (x1 - C) / (D - C)    # unit-scale at fire-wise bounds
  return(qlogis(x2))          # logit-scale at fire-wise bounds.
  
}

# To get the derivative from wolframalpha, we need to write it down in a single 
# step
logit_translate_check <- function(x, A, B, C, D) {
  log( (((1/(1 + exp(-x))) * (B-A) + A) - C) / (D - C) ) -
    log(1 - (((1/(1 + exp(-x))) * (B-A) + A) - C) / (D - C))
}

# logit_translate(0, 2, 100, 10, 80)
# logit_translate_check(0, 2, 100, 10, 80)

# derivative of logit_translate
logit_translate_d <- function(x, A, B, C, D) {
  -(exp(x) * (A - B) * (C - D)) / 
    ((A + B * exp(x) - C * (exp(x) + 1)) * (A + B * exp(x) - D * (exp(x) + 1)))
}
...
#   Fires in rows. (or whatever required transform).
...
update_ranef <- function(Y, mu, Sigma, mu0, Sigma0, Ytry, steps_bounds, 
  ...
  for(j in 1:J1) {
    ...
    #   hierarchical prior +
    #   stage1 prior * (-1) +
    #   log-jacobian for exp_logit(steps) transform * (-1) +
    #   area likelihood
    # The log(abs(jacobian)) is part of the stage1 prior, so it is negated too.
    
    # (THE JACOBIAN HAD THE OPPOSITE SIGN IN ALL RUNS BEFORE THE STEPS 
    # WERE MODELLED AT LOGIT SCALE! IT WAS A BUG.)
...
# REVISAR: Y e Ytry, llevarán los steps en la escala logit o no? -------

# The same, but with logitnormal model for steps. Y and Ytry carry the steps
...
# different bounds. So their jacobian is similar, but uses different bounds.
...
# areaL: lower bound for log-area.
# area_coef: b1, b2 and s2 for area, to compute likelihood conditional on steps.
update_ranef2 <- function(Y, mu, Sigma, mu0, Sigma0, Ytry, 
  ...
  for(j in 1:J1) {
    ...
    # hierarchical priors
    Yold_h <- Yold_h # duplicate to transform steps
    Ynew_h <- Ynew_h  
