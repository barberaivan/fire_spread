...
# // Landscape layers names in FireSpread package (OLD OLD OLD)
# enum land_names {
#   vfi,
#   tfi,
#   elev,
...
for(i in 1:n_fires) {
  ...
  # Compute NDVI terms (squared differences). The shrubland and grassland are
  ...
  ndvi_term[veg_values == 0] <- (ndvi_dt[veg_values == 0] -
                                 ndvi_params$optim_ndvi_mean["Forest"]) ^ 2

  ndvi_term[veg_values == 1] <- ndvi_params$pi_ndvi_mean["Shrubland"] *
                                (ndvi_dt[veg_values == 1] -
                                 ndvi_params$optim_ndvi_mean["Shrubland"]) ^ 2

  ndvi_term[veg_values == 2] <- ndvi_params$pi_ndvi_mean["Grassland"] *
                                (ndvi_dt[veg_values == 2] -
                                 ndvi_params$optim_ndvi_mean["Grassland"]) ^ 2
