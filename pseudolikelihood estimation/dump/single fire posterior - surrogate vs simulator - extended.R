...
# code to compare the single-fire posteriors.

# Compare the following sampling procedures:

# * fire simulator
# * surrogate without uncertainty, violating Jensen
# * surrogate without uncertainty, taking care of Jensen with momentsLogitnorm
# * surrogate with noise,
# * surrogate with noise and parameters uncertainty.

# Do the same for a few fires.

# If the comparison is bad, try what happens if we use a different smooth
# acceptance kernel. Now, probability is just
# 1 - (1 - overlap) = overlap.
# But if we care for the highest overlap area, we could use a normal kernel:
# exp(-((1 - overlap) / scale) ^ 2)
# with scale ~ 0.2

# Then, the surrogates might perform better, because they take more samples at
