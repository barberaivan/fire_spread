...
prior_dist <- function(mu_int = 0, sd_int = 5,
                       r_slope = 0.04,
                       r_fi = 0.15,
                       r_wind = 0.3,
...
loglik_update_cumulative <- function(
  ...
  particles_data_new$in_bounds <- in_bounds(particles_data_new,
                                            ...
                                            var_factor = 2,
                                            edge_prop_upper = 0.05)
  # neglect fires touching the edge
...
p1 <- loglik_plot(w1, varying = "all", color_point = "in_bounds")
...
p2 <- loglik_plot(w2, varying = "all", color_point = "in_bounds")
...
p3 <- loglik_plot(w3, varying = "all", color_point = "in_bounds")
...
w4 <- loglik_update_cumulative(
  ...
  n_pw = 1000,
  n_fit = 800,
  loglik_tolerance = 0.5
)
p4 <- loglik_plot(w4, varying = "all", color_point = "in_bounds")
...
p5 <- loglik_plot(w5, varying = "all", color_point = "in_bounds")


wave_test <- list(w5 = w5, similarity_bounds = bb, 
                  fire_id = full_data$fire_id)
# used the qlogis link
saveRDS(wave_test, file.path("files", "wave_test_1999_25j.rds"))

# probar con este mañana


# n available
sum(w5$particles_data$ll > -3 & w5$particles_data$in_bounds == 1)
# 557 puntos... parece que zafa.
...
p6 <- loglik_plot(w6, varying = "all", color_point = "in_bounds")

# n available
sum(w6$particles_data$ll > -3 & w6$particles_data$in_bounds == 1)
# 795 puntos... parece que zafa.
wave_test <- list(w6 = w6, similarity_bounds = bb, 
                  fire_id = full_data$fire_id)
# used the qlogis link
saveRDS(wave_test, file.path("files", "wave_test_2008_3.rds"))


# ya en el 7mo GP le re cuesta conseguir más partículas, tarda muchísimo porque
# el gp se vuelve muy puntudo
...
p7 <- loglik_plot(w7, varying = "all", color_point = "in_bounds")

# Este se cuelga buscando particulas
# porque el anterior ya era muy puntudo.
# poner un limite al while de more_particles para que no pase esto
...
p8 <- loglik_plot(w8, varying = "all", color_point = "in_bounds")

# usar loglik tolerance 3, 2, 1, 1, 1

# usando log anda bastante mal. Los GP predicen valores reeeee altos
# que no tienen sentido, y en la zona bounded.

# y usando logit? Lo mismo

# fittear el gp usando las bounded también, así puede encontrar la puntita.

# probar con esto cómo se ajusta el MVN
