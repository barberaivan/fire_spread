...
# Run cholila after break -------------------------------------------------

# It used too much RAM, so got errors. Run for n_sim = n_rep = 10 instead of
# 20, and with ncores = 8
...
# Load previous runs
ff <- list.files(target_dir, "2015_50-wave-")
wave1 <- do.call("rbind", lapply(1:length(ff), function(j) {
  wprev <- readRDS(file.path(target_dir, ff[j]))
  print(nrow(wprev))
  return(wprev)
})) # OK
...
wave2 <- explore_likelihood_iterate(
  n_waves = 20, data = wave1, n = 500, p_best = 0.15,
  var_factor = c(1.5, 2, 1),
  support = sup, spread_data = spread_data, n_rep = n_rep,
  centre = "global", sobol = TRUE, phase = "search_higher",
  write = write_file, fire_name = fire_name
)
...
for(f in 1:n_fires) {
  ...
  like_sim <- rbind(wlist$abc_prob_check,
                    wlist$like_sim[, colnames(wlist$abc_prob_check)])

  like_sim$overlap <- like_sim$metrics[, "ov"]

  # waveplot including test data
  nn <- paste(wlist$fire_name, "-waveplot_overlap.png", sep = "")
  png(filename = file.path(target_dir, nn),
      width = 22, height = 11, units = "cm", res = 300)
  wave_plot(like_sim, "overlap", alpha = 0.05,
            rc = c(2, 3), tit = wlist$fire_name)
  dev.off()
}


# Simulate overlap in regions of high overlap -----------------------------


# I don't trust the previous exploration, as it used the abc_prob instead of
# overlap. From the overlap POV, many waveplots are quite rare.
