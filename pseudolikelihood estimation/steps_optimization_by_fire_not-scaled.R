...
get_bounds <- function(fire_data, n_sim = n_rep) {
  ...
  coef_burn_all <- c(rep(1e6, 3), rep(0, n_coef - 3), 0) # intercepts, slopes, steps
  coef_burn_none <- c(rep(-1e6, 3), rep(0, n_coef - 3), 1)
...
# for(f in 1:length(filenames)) { # electricity went down
for(f in 44:length(filenames)) {
  ...
  # clean
  remove(spread_data, wave1, wave2)
  gc()
}
#endline


# Improve simulation in parallel: -----------------------------------------

# When a core has errors, it does not return a result. The function should be
# robust against this. Perhaps by returning the overlap values alongside the
# particle used.
