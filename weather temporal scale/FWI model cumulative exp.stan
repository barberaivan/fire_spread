...
parameters {
  ...
  real rho_logit_raw; // temporal correlation
}
...
transformed parameters {
  ...
  real rho_logit = rho_logit_raw * prior_sd_rho;
  real rho = inv_logit(rho_logit);
  ...
  for(t in 1:N_times)
    weights_improper[t] = rho ^ ((t-1) / time_scale_exp);
