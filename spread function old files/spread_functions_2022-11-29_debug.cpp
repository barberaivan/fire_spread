...
IntegerVector simulate_fire_cpp(NumericMatrix landscape, 
  ...
  // std::vector<int> cell_ids = seq(0, n_cell - 1); // no anda
  ...
  // IntegerVector burning_cells(n_cell);
  ...
  // define burning cells outside loop, size = 0.
  // IntegerVector burning_cells; 
  // std::vector<int> burning_cells; 
  ...
  while(burning_size > 0) {
    ...
    Rcout << "burning size start " << burning_size << "\n";  
    
    // IntegerVector burning_start = burning_cells[Range(0, burning_last - 1)];
    
    // // Get burning cells ids
    // burning_cells.resize(burning_size); 
    // // wrap(burning_cells);
    // burning_cells = cell_ids[burn == 1];
    // wrap(burning_cells);
    // Rcout << "burning cells start\n" << burning_cells << "\n";  
    
    // Turn burning ones into burned
    ...
    for(int b = 0; b < burning_size; b++) {
      ...
      Rcout << "neigh_b start empty:\n" << neigh_b << "\n";
      Rcout << "positions  start:\n" << positions << "\n";
      
      for(int i = 0; i < 8; i++) neigh_b[i] = neighbours(b, i); 
      Rcout << "neigh_b start full:\n" << neigh_b << "\n";
      ...
      for(int n = 0; n < 8; n++) {
        ...
        // bool out_of_range = R_IsNA(neigh_b[n]);
        bool out_of_range = neigh_b[n] == -2147483648;
        Rcout << "out_of_range: " << out_of_range << "\n";
        ...
        // }
        // 
        // Rcout << "texto de prueba\n";
        // 
        // // ----
        // 
        // int focal_cell = neigh_b[n];
        // 
        // // ----
        // 
        // bool currently_burning = burning[focal_cell] == 2;
        // Rcout << "currently_burning: " << currently_burning << "\n";
        // bool not_burnable = burnable[focal_cell] != 1;
        // Rcout << "not_burnable: " << not_burnable << "\n";
        // bool already_burned = burned[focal_cell] == 1;
        // Rcout << "already_burned: " << already_burned << "\n";
        // 
        // bool invalid = R_IsNA(neigh_b[n]) |            // neighbour out of range
        //   (burned[neigh_b[n]] == 1) |     // already burned
        //   (burning[neigh_b[n]] == 2) |    // currently burning
        //   (burnable[neigh_b[n]] != 1);     // not burnable
        // 
        // Rcout << "invalid: " << invalid << "\n";
        // 
        // if ( invalid
        //   // R_IsNA(neigh_b[n]) |            // neighbour out of range
        //   // (burned[neigh_b[n]] == 1) |     // already burned
        //   // (burning[neigh_b[n]] == 2) |    // currently burning
        //   // (burnable[neigh_b[n]] != 1)     // not burnable
        // ) {
      ...
      Rcout << "neigh_b without na:\n" << neigh_b << "\n";
      Rcout << "positions  without na:\n" << positions << "\n";
      ...
      if(neigh_b.length() > 0) {
        ...
        IntegerVector temp(neigh_b.size());
        temp = spread_around_cpp(data_burning, 
                                 data_neighbours,  
                                 coef,
        ...
        burning[neigh_b] = temp;//spread_around_cpp(data_burning, 
                                             // data_neighbours,  
                                             // coef,
                                             // positions, // columns of valid neighbours
                                             // wind_column,
    ...
    burning[burning == 2] = 0; // turn off previouly burned
    burned[burning == 1] = 1;  // set as burned the currently burning.
    
    burning_size = sum(burning);
    Rcout << "burning size end " << burning_size << "\n";  
    ...
    // parece que no le gusta que la asignación tenga doble subsetting
    // burning_cells = cell_ids[burn == 1];
  } // end while 
...
// functions to test resizing.

// [[Rcpp::export]]
NumericMatrix give_sized(int n_row, int n_col) {
  NumericMatrix m(n_row, n_col);
  return m;
}
...
// [[Rcpp::export]]
NumericMatrix give_last(double min = 5, double max = 1000, int iter = 100) {
  ...
  while(i <= iter) {
    ...
    Rcout << "iter = " << i << "\n";  
    int dim = ceil(R::runif(min, max));
    
    Rcout << "dim = " << dim << "\n";  
    NumericMatrix temp(dim, dim);
...
// Esto funciona bien, no sé por qué le molestan tanto las otras cosas. Quizás 
// no sea un problema de espacios, si no otra cosa.