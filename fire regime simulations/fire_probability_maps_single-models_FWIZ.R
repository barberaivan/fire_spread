...
plot(pnnh_rast$igprob_h, range = c(0, 1))
plot(pnnh_rast$igprob_l, range = c(0, 1))
...
# design matrix
Xesc <- model.matrix.lm(~ vfi + tfi + drz + dhz - 1, data = pnnh_df,
                        na.action = "na.pass")
...
for(i in 1:npost) {
  ...
  pp <- plogis(ranef["intercept"] + Xspread_complete %*% ranef[c("vfi", "tfi")])
...
writeRaster(pnnh_rast, file.path("data", "pnnh_images", "pnnh_data_120m_buff_10000_ig-esc-spread-prob_FWIZ.tiff"))