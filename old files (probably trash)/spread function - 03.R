# Write in base R language 2 spread functions, vectorizing rasters or not.
library(bench)
library(terra)
library(tidyverse)
...
while(length(burning_cells) > 0) {
  ...
  for (i in length(burning_cells)) {
    burned[burning_cells[i]] <- 2
  }
  ...
  for(b in 1:length(burning_cells)) {
    ...
    # simulate spread from burning_cell b
    if(length(neighbours) > 0) {
      
      # compute probabilities
      probs <- spread_prob(data_burning, data_neighbours, cols_use = cols_use)
      
      # sample fire occurrence
      for(n in 1:length(neighbours)) {
        burned[neighbours[n]] <- rbinom(n = 1, size = 1, prob = probs[n])
      }
    }
  ...
  # update burning_cells (no loop) 
  # no tiene sentido intentarlo en R si en cpp es muy distinto.
  # mejor ir a cpp
  burning_maybe <- numeric(burning_cells * 8)
...
# function to spread
spread_recursive <- function(burning_cells_init, burned, 
    ...
    for(b in 1:length(burning_cells)) {
      ...
      if(length(neighbours) > 0) {
        ...
        burned[neighbours] <- rbinom(
          ...
          prob = spread_prob(data_burning, data_neighbours, cols_use = cols_use)
        )
      }
...
spread_recursive(burning_cells_init = ig_location,
                 burned = numeric(nrow(predmat)),
