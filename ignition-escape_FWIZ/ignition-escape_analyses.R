...
logistic_predict <- function(pdata, xname, bname) {
  if(xname == "fwi") {
    ...
    bb <- as.matrix(escmod, c("a", bname, "b_fwi")) |> t()
    x <- pdata[, xname]
    XX <- cbind(rep(1, length(x)), x, rep(fwi_cumulative_mean, length(x)))
    pmf <- plogis(XX %*% bb)
  }
  
  summ <- apply(pmf, 1, mean_ci) |> t() |> as.data.frame()
  return(cbind(pdata, summ))
}
...
prior_U_mean <- upper_igr_mean / study_area_size # = 14 / s a size
prior_U_sd <- upper_igr_sd / study_area_size     # = 14 / s a size
...
sdata <- list(
  ...
  # condition_unk = which(condition_like == 3), # to save id proportion
  # N_condition_unk = length(which(condition_like == 3)), 
  fwi_mat = fwi_mat,
  study_area_size = study_area_size, # 7161.577
  ...
  cause = ig$cause_num,
  # N_cause_unk = sum(ig$cause_num == 3),
  # cause_unk_ids = 
  fort_id = ig$fort_id,
  year_id = ig$year_id,
...
#   cores = 8, chains = 8, iter = 2500, warmup = 1500,
...
## 12536.5 / (60*60) = 3.48 h 

igmod <- readRDS(file.path("files", "ignition_FWIZ", "ignition_model_samples.rds"))
sig <- summary(igmod)[[1]]
min(sig[, "n_eff"]) # 2478.018 
max(sig[, "Rhat"])  # 1.002096

pairs(igmod, pars = c("a[1]", "U[1]", "b[1]", "phi_a[1]", "phi_b[1]"))
pairs(igmod, pars = c("a[2]", "U[2]", "b[2]", "phi_a[2]", "phi_b[2]"))
...
mcmc_dens(igmod, pars = c("b[1]", "b[2]"),
          facet_args = list(scales = "fixed", nrow = 2))
mcmc_dens(igmod, pars = c("U[1]", "U[2]"),
          facet_args = list(scales = "fixed", nrow = 2))

mcmc_dens(igmod, pars = c("phi_a[1]", "phi_a[2]",
                          "phi_b[1]", "phi_b[2]"),
...
fwi_summ <- as.vector(fwi_mat %*% wpoint)

np <- 200

# run this plot using the first and second fwiseq definition, to 
# extend the range or not
fwiseq <- seq(min(fwi_summ), max(fwi_summ), length.out = np)
# fwiseq <- seq(min(fwi_summ), max(fwi_summ) + diff(range(fwi_summ)), length.out = np)
# fwiseq_ori <- fwiseq * fwi_sd + fwi_mean

# alpha and beta for humans (h) and ligthning (l)
coef_ir <- as.matrix(igmod, pars = c("a[1]", "b[1]", "U[1]",
                                     "a[2]", "b[2]", "U[2]")) |> t()
mumat_h <- mumat_l <- matrix(NA, np, npost)

for(i in 1:npost) {
  mumat_h[, i] <- 
    plogis(coef_ir["a[1]", i] + coef_ir["b[1]", i] * fwiseq) * coef_ir["U[1]", i]
  mumat_l[, i] <- 
    plogis(coef_ir["a[2]", i] + coef_ir["b[2]", i] * fwiseq) * coef_ir["U[2]", i]
}

lamh <- apply(mumat_h, 1, mean_ci) |> t() |> as.data.frame()
laml <- apply(mumat_l, 1, mean_ci) |> t() |> as.data.frame()
...
ggplot(lam, aes(fwi, mean, ymin = lower, ymax = upper,
  ...
  geom_vline(xintercept = max(fwi_summ), linetype = "dashed",
             linewidth = 0.3) +
  geom_ribbon(color = NA, alpha = 0.3) +
  geom_line() +
...
ig2$escape <- as.numeric(ig2$area_impute > 0.09)
...
# smodel_esc <- stan_model(file.path("ignition-escape_FWIZ", "escape_model.stan"))
# escmod <- sampling(
#   smodel_esc, data = sdata_esc, seed = 1596142, refresh = 200,
#   # cores = 1, chains = 1, iter = 5
#   cores = 8, chains = 8, iter = 2000, warmup = 1000
# )
# saveRDS(escmod, file.path("files", "ignition_FWIZ", "escape_model_samples.rds"))
...
# Escape model predictions ------------------------------------------------

# FWI weights ____________________________________________________________

ls_hat <- as.numeric(as.matrix(escmod, "ls"))
npost <- length(ls_hat)
...
# Compute average FWI:
fwi_cumulative_draws <- fwi_points %*% wn[ids_norm, ]
fwi_cumulative <- rowMeans(fwi_cumulative_draws)
fwi_cumulative_mean <- mean(fwi_cumulative)

# the mean is used to fix this variable while making predictions:
# 0.5485897. The vector is used to create the sequence.
...
pd_ndvi$vfi <- vfi_calc(pd_ndvi$vegnum, pd_ndvi$ndvi)

prob_ndvi <- logistic_predict(pd_ndvi, "vfi", "b_vfi")
...
pd_veg <- data_summ$ndvi[, 1:2]
colnames(pd_veg)[2] <- "ndvi"
pd_veg$vegnum <- as.numeric(pd_veg$vegetation)
pd_veg$vfi <- vfi_calc(pd_veg$vegnum, pd_veg$ndvi)

prob_veg <- logistic_predict(pd_veg, "vfi", "b_vfi")
...
esc_veg <-
  ...
  theme(axis.title.y = element_blank(),
        ...
        legend.text = element_text(margin = margin(l = 0.3, unit = "mm")),
...
prob_topo <- logistic_predict(pd_topo, "tfi", "b_tfi")
...
prob_roads <- logistic_predict(pd_roads, "drz", "b_drz")
...
prob_humans <- logistic_predict(pd_humans, "dhz", "b_dhz")
...
pd_fwi <- data.frame(
  fwi = seq(quantile(fwi_cumulative, prob = 0.01),
            quantile(fwi_cumulative, prob = 0.99), length.out = nseq)
)

prob_fwi <- logistic_predict(pd_fwi, "fwi", "b_fwi")
...
leg <- ggpubr::as_ggplot(ggpubr::get_legend(esc_veg)) + 
  theme(plot.margin = margin(t = 0, b = 5, unit = "mm"))

plot_out <- 
  wrap_elements(
  esc_veg + theme(legend.position = "none") + 
  esc_ndvi + 
  leg +
  # ggpubr::as_ggplot(ggpubr::get_legend(esc_veg)) + 
  # ggplot() + geom_blank() + theme_void() + 
  esc_elev + esc_north +
  esc_roads + esc_humans +
  esc_fwi +
  plot_layout(design = layout, heights = c(10, 1.5, 10, 10, 10))
  ) +
  labs(tag = "Escape probability (%)") +
  theme(
