...
parameters {
  ...
  vector<lower=0>[nig] phi_a_raw;
  vector<lower=0>[nig] phi_b_raw;
  real<lower=0> ls_raw;
...
transformed parameters {
  ...
  vector[nig] phi_a = phi_a_raw * prior_phi_a_sd;      // dispersion
  vector[nig] phi_b = phi_b_raw * prior_phi_b_sd;      
  matrix[nfort, nig] phi;
  ...
  // lambda and dispersion for ignition rates
  for(i in 1:nig) {
    lambda[, i] = inv_logit(a[i] + b[i] * fwi) * U[i] * study_area_size;
    phi[, i] = phi_a[i] + phi_b[i] * lambda[, i];
  }
...
model {
  ...
  phi_a_raw ~ std_normal();
  phi_b_raw ~ std_normal();
  ...
  for(i in 1:nfort) {
    ...
    if(condition[i] == 1) {
     ig_counts[i, 1] ~ neg_binomial_2(lambda[i, 1], phi[i, 1]); // human
     ig_counts[i, 2] ~ neg_binomial_2(lambda[i, 2], phi[i, 2]); // lightning
    }
    ...
    if(condition[i] == 2) {
     ...
     for(k in 0:nunk) {
       ...
       ll[k+1] = neg_binomial_2_lpmf(n_human | lambda[i, 1], phi[i, 1]) +
                 neg_binomial_2_lpmf(n_light | lambda[i, 2], phi[i, 2]);
     }
